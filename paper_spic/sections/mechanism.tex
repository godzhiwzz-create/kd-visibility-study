...
\textbf{Supplementary controlled validation.} Beyond the main correlation analysis, our Phase 1 follow-up experiments inject controlled synthetic occlusion at multiple ratios and compare student-only, logit-only, and localization-only branches. The purpose of this follow-up is narrow: to test whether worsening occlusion consistently weakens the relative benefit of localization-oriented transfer. The observed trend supports this expectation, providing additional intervention-style evidence that M2 captures a real part of the failure process. We therefore treat Phase 1 as supplementary validation that makes the occlusion mechanism more concrete, not as a replacement for the main branch-wise observation and mechanism analysis.
...
Occlusion (within M2) & Occlusion degrades localization & $\mathbf{-0.989}$ & $\mathbf{0.0015}$ & \textbf{Directly supported} & Prioritize occlusion-aware methods; supported further by Phase 1 control \\
\bottomrule
\end{tabular}
\end{table*}
...
This finding reframes the problem: KD under fog is not primarily about distribution alignment (M1) or confidence calibration (M3), but about \emph{information loss through occlusion}. When atmospheric scattering hides object boundaries, the spatial information required for accurate localization becomes fundamentally unavailable. This explains why localization distillation, which explicitly transfers spatial knowledge, is most affected; and why other branches that rely on classification probabilities or feature patterns may partially compensate, but cannot fully overcome the spatial information deficit. Since localization distillation is the primary source of KD performance gain under mild conditions (\Cref{tab:gain_matrix}), its degradation under occlusion directly explains the collapse of overall KD effectiveness under visibility degradation. The supplementary Phase 1 control results are consistent with this reading and help solidify M2, but they do not supersede the main evidence presented in this section.
