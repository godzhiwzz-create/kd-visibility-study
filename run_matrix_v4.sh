#!/bin/bash
# 批量运行5×3实验矩阵 - v4优化版（充分利用RTX 5090）
...
OUTPUT_ROOT=$PROJECT_ROOT/outputs_v4
...
echo "=========================================="
echo "5×3实验矩阵 - v4优化版 (RTX 5090全速)"
echo "总实验数: $TOTAL"
echo "每个实验轮数: $EPOCHS"
echo "Student模型: yolov8m (更大模型)"
echo "Batch size: 32 (充分利用32GB显存)"
...
for branch in "${BRANCHES[@]}"; do
    for vis in "${VISIBILITIES[@]}"; do
        ...
        # 运行实验
        echo "  -> 开始训练 (优化版)..."
        $PYTHON scripts/train_yolo_v4.py \
            --kd-branch $branch \
            --visibility $vis \
            --epochs $EPOCHS \
...
echo "{" > $SUMMARY_FILE
echo "  \"version\": \"v4_optimized\"," >> $SUMMARY_FILE
echo "  \"timestamp\": \"$(date -Iseconds)\"," >> $SUMMARY_FILE
echo "  \"gpu\": \"RTX 5090 32GB\"," >> $SUMMARY_FILE
echo "  \"epochs\": $EPOCHS," >> $SUMMARY_FILE
echo "  \"batch_size\": 32," >> $SUMMARY_FILE
echo "  \"student_model\": \"yolov8m\"," >> $SUMMARY_FILE
