...
TASKS_PATH = WORK_ROOT / 'scripts/faster_rcnn_boundary_check' / 'phase4_occaware_tasks.json'
STATE_PATH = WORK_ROOT / 'scripts/faster_rcnn_boundary_check' / 'phase4_occaware_state.json'
LOG_DIR = WORK_ROOT / 'logs' / 'faster_rcnn_occaware'
RUNNER_PID = WORK_ROOT / 'scripts/faster_rcnn_boundary_check' / 'phase4_occaware_runner.pid'
...
def main():
    ...
    try:
        while True:
            state = load_state()
            mark_completed_from_disk(state)
            pending = [t for t in state['tasks'] if t.get('status') != 'completed']
            if not pending:
